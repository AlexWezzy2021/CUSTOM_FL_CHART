...
class BarChartData extends AxisChartData with EquatableMixin {
  ...
  BarChartData({
    ...
    if (typeChart == TypeChart.Sleep && maxY != 0 && minY != 0) {
      this.timeStart = minY;
      this.timeFinish = maxY;
      maxY = ((Duration(milliseconds: maxY.toInt()).inHours - Duration(milliseconds: minY.toInt()).inHours) * 60).toDouble();
      minY = 0;
    }
  ...
  void initSuperMinMaxValues(double maxY,
      double minY,) {
  ...
  BarChartData copyWith({
    ...
    return BarChartData(
      ...
      divisionValue: divisionValue ?? this.divisionValue,
      typechart: typeChart,
...
class BarChartGroupData with EquatableMixin {
  ...
  @required
  final double posMinY;

  @required
  final double posMaxY;
  ...
  BarChartGroupData copyWith({
    ...
    return BarChartGroupData(
      x: x ?? this.x,
      minY: minY ?? this.posMinY,
      maxY: maxY ?? this.posMaxY,
      barList: barList ?? this.barList,
...
class BarTouchedSpot extends TouchedSpot with EquatableMixin {
  ...
  BarTouchedSpot(BarChartGroupData touchedBarGroup,
...
class BarItem with EquatableMixin {
  ...
  /// Used for equality check, see [EquatableMixin].
  @override
  List<Object> get props => [top, bottom, primaryColor];
}
